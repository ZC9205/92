...
图像的亮度和色彩范围较低，视觉效果一般
亮度一般为100-300尼特
色彩深度限于8位，纹理图的RGB通道每个占用8Bit，数值区间为0-255

### HDR（高动态范围）
图像的亮度和色彩显著高于SDR，视觉效果较好
亮度可达到1000-10000尼特，白色更亮，黑色更深
色彩深度可以达到10位甚至12位，纹理图的RGB通道占用可达10位甚至16位，数值区间可达到0-1024甚至0-4096

### Tonemapping（色度映射算法）
当原图为HDR但设备只能显示SDR时（如Unity的Camera只支持SDR作为输出），将HDR转换成SDR，并尽量多保留细节保证图像不失真的算法
