...
### Batch
将满足条件物体的数据合并后并提交渲染的次数。需要注意的是有些合批方式并不会减少该次数，比如SRP Batch执行时虽然会减少渲染状态切换（Set Pass Call）及绑定物体数据的时间，但依然是每个物体单独提交渲染数据，因此不会减少Batch次数
...
##### 注意
SRP Batch合批方式并不像其他合批方式一样会合并满足条件的物体数据，实际渲染时还是依次绑定每个物体属性并单独渲染，因此并不会减少Batch和DrawCall
